...
export const SocketProvider: React.FC<SocketProviderProps> = ({ children }) => {
    ...
    useEffect(() => {
        socketRef.current = io(process.env.DEV_URL);
        ...
        socket.on('event:delete',  (mID: string) => {
            setData((prevData) => {
                const updatedData = prevData.filter((msgdata) => msgdata.mID !== mID);
                return updatedData;
            });            
        });
        ...
        socket.on('event:receive', (data: MessageData) => {
            setData((prevState) => [data, ...prevState]); // Add the new data at the front of the array
        });
