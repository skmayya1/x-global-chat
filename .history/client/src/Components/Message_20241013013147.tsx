...
const Message: React.FC<MessageProps> = ({ msg, id, ReplyHandler, PrevMsgerID, setPrevMsgerID }) => {
    ...
    const [namereq, setnamereq] = useState<boolean | null>(true);

    // Check if we should display the sender's name
    useEffect(() => {
        const isSameSender = msg.kinderId === PrevMsgerID;
        setnamereq(!isSameSender);
        setPrevMsgerID(msg.kinderId);
    }, [msg.kinderId, PrevMsgerID, setPrevMsgerID]);
