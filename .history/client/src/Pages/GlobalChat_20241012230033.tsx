...
const GlobalChat = () => {
  ...
  return (
    <div className="h-screen w-full flex flex-col relative">
      ...
      {ReplyData.ReplyToId && (
        <div className="absolute bottom-0 right-0 bg-zinc-800 text-white p-2 rounded-lg flex gap-2">
          <div className="font-mono font-extralight text-xs">Replying to: {ReplyData.ReplyToName}</div>
          <div className="font-mono font-extralight text-xs">{ReplyData.ReplyToText}</div>
          <button onClick={() => setReplyData({ ReplyToId: '', ReplyToName: '', ReplyToText: '' })} className="p-1 rounded-full flex items-center justify-center">
      ...
      <form onSubmit={SubmitHandler} className="flex w-full py-1 items-center justify-center mt-1 gap-2 bg-[#121212]">
        <input
