...
interface MessageProps {
    ...
    updatePrevMsgerID: (value: string ) => void;
}

const Message: React.FC<MessageProps> = ({ msg, id, ReplyHandler, prevMsgerID, updatePrevMsgerID }) => {
    const showName = isNameShown(prevMsgerID, msg.kinderId, id);

    useEffect(() => {
        if (prevMsgerID !== msg.kinderId) {
            updatePrevMsgerID(msg.kinderId);
        }
    }, [msg.kinderId , prevMsgerID, updatePrevMsgerID]);
