  ...
  return (
    <div className="h-screen w-full flex flex-col relative">
      ...
      <div className="overflow-auto h-full w-full flex flex-col justify-end px-1.5">
        {data?.map((msg, index) => (
         <Message key={index} msg={msg} id={user?.id || ''} />
        ))}
        <div ref={chatEndRef} />
      </div>
      <form onSubmit={SubmitHandler}
        ...
        <button hidden={message.length === 0 ? true : false} type="submit" className="bg-white p-2.5 rounded-full flex items-center justify-center">
