  ...
  return (
    <div className="h-screen w-full flex flex-col relative">
      ...
      <div className="overflow-auto h-full w-full flex flex-col">
        {data?.map((msg, index) => (
          <div key={index} className={`chat  ${msg.socketId === socketID ? 'chat-end' : 'chat-start'}`}>
            { msg.socketId !== socketID ?
              <div className="h-9 w-9 bg-white rounded-full">
            </div> : ''}
            <div className="bg-zinc-900 p-2 rounded-lg">{msg.message}</div>
            {msg.socketId == socketID ?
              <div className="h-9 w-9 bg-white rounded-full mr-2">
              </div> : ''}
          </div>
        ))}
        <div ref={chatEndRef} />
