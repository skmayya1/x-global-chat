...
const GlobalChat = () => {
  ...
  const { memberCount, sendMessage, data = [] ,deleteMessage } = useContext(SocketContext) || {};
  ...
  const DeleteHandler = (mID: string) => {
    console.log('DeleteHandler:', mID);
        ...
        {Object.entries(groupedMessages).map(([date, messages]) => (
          <div key={date} className="mb-2">
            ...
            {messages.map((msg, index) => (
              <Message key={index} msg={msg} id={user?.id || ''} ReplyHandler={ReplyHandler} DeleteHandler={DeleteHandler} />
            ))}
          </div>
        ))}
