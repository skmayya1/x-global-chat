...
interface MessageProps {
    ...
    id: string;
    namereq: string | null;
    setnamereq: React.Dispatch<React.SetStateAction<string>>;
    ReplyHandler: (name: string, id: string, text: string) => void;
}

const Message: React.FC<MessageProps> = ({ msg, id, ReplyHandler, namereq ,setnamereq }) => {
    const [showName, setshowName] = useState<boolean>(true)
    useEffect(() => {
        if (msg.kinderId === namereq && msg.kinderId !== id) {
            setshowName(false);
        } else {
            setshowName(true);
        }

        // Update the requested name ID for the next messages
        setnamereq(msg.kinderId);
    }, [msg, id, namereq, setnamereq]);
                ...
                msg.kinderId !== id ?
                    ...
                    <div className="flex items-center justify-center gap-2 mb-1">
                        ...
                        <div className="bg-zinc-800 rounded-lg max-w-[32vh] px-2 py-1 overflow-hidden">
                            {showName && <div className={`font-medium text-sm ${senderColor}`}>{msg.sender_name}</div>}
                            {msg.ReplyToId && 
