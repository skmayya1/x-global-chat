...
const GlobalChat = () => {
  ...
  useEffect(() => {
    scrollToBottom(); // Scroll to the bottom when data changes
  }, [data]);
  ...
  const handleUserScroll = () => {
    ...
    const nearBottom = scrollHeight - scrollTop <= clientHeight + 50; // Threshold to determine "near bottom"
  ...
  return (
    <div className="h-screen w-full flex flex-col relative">
      <div className="flex items-center justify-around gap-10 border">
        <div>
          <CiLogout />
        </div>
        <div className="h-3 border-b border-zinc-800 text-white flex items-center justify-center flex-col gap-0.5 py-10">
      ...
      { isLoading ? <div className="flex items-center justify-center h-full w-full">Loading...</div> :
        <div className="overflow-y-scroll self-auto h-full w-full flex flex-col px-1.5" onScroll={handleUserScroll}>
