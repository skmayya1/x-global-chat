...
export const SocketProvider: React.FC<SocketProviderProps> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        socket.on('event:delete', (mID: string) => {
            setData((prevState) => {
                const messageExists = prevState.some((msg) => msg.mID === mID);
                if (!messageExists) {
                    console.warn(`Message with ID ${mID} does not exist.`);
                    return prevState;
                }
                console.log('Message deleted:', mID);
                return prevState.filter((msg) => msg.mID !== mID);
            });

            // Optionally, notify the user about the deletion.
            alert(`Message with ID ${mID} has been deleted.`);
        });
