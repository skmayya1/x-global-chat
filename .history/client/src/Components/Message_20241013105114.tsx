...
const Message: React.FC<MessageProps> = ({ msg, id, ReplyHandler, prevMsgerID, setprevMsgerID }) => {
    ...
    useEffect(() => {
        if (msg && id) {
            // Determine if the name should be shown based on the previous message sender
            const shouldShowName = isNameShown(prevMsgerID, msg.kinderId, id);
            setShowName(shouldShowName);
            // Update the previous message sender ID to the current message's sender ID
            setprevMsgerID(msg.kinderId);

            // Debug logs to see the current state
            console.log("prevMsgerID:", prevMsgerID);
            console.log("msg.kinderId:", msg.kinderId);
            console.log("id:", id);
            console.log("showName:", shouldShowName);
        }
    }, [msg.kinderId, id, setprevMsgerID]);
