        ...
        <div className={`chat group ${msg.kinderId === id ? 'chat-end' : 'chat-start'}`}>
            {
                msg.kinderId !== id ?
                    ...
                    :
                    <div id={msg.mID} className="flex gap-1  relative"> {/* Added 'relative' for positioning */}
                        <div onClick={
                            () => {
                                if (DeleteHandler) {
                        ...
                        <div className="bg-zinc-800 rounded-lg max-w-[32vh] px-2 py-1 overflow-hidden mb-1">
                            {msg.ReplyToId && (
                                <a href={msg.ReplyToId} className="bg-zinc-700 min-h-14 min-w-[28vw] mb-1 border-l-2 rounded-lg flex flex-col items-start p-2 overflow-hidden">
                                    <div className="text-sm text-white font-base">{msg.ReplyToNAme?.split(' ')[0]}</div>
