...
const GlobalChat = () => {
  ...
  const updatePrevMsgerID = (newMsgerID: string) => {
    console.log('newMsgerID:', newMsgerID);
    setprevMsgerID((prev) => (prev !== newMsgerID ? newMsgerID : prev));
  ...
  return (
    <div className="h-screen w-full flex flex-col relative">
      ...
      <div className="overflow-y-scroll h-full w-full flex flex-col  px-1.5">
        {data.map((msg, index) => (
          <Message key={index} msg={msg} id={user?.id || ''} ReplyHandler={ReplyHandler} prevMsgerID={prevMsgerID} setprevMsgerId={updatePrevMsgerID} />
        ))}
